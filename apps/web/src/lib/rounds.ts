/**
 * ROUNDS — the wire shapes of the proactive surface, read into the shapes
 * the surface renders.
 *
 * Rounds is the one screen in this product with no asker in the room. An
 ...
 *
 * **A tile without its integrity atom is not parsed.** `RoundsTileIntegrity`
...
/** `RoundsPresentation` — how the analyst chose to see this watch. */
export type RoundsPresentation = "chart" | "finding" | "worklist_slice" | "scalar";

/** `RoundsWindowMode` — what re-running this watch's window means. */
export type RoundsWindowMode = "relative" | "absolute" | "anchored";

/**
 * Four Rounds shapes are NOT defined here, and the split is the backend's
 * own: `RoundsWatchModel`, `WatchDeclarationPayload`, `TimeToImpactPayload`
 * and `lead_status` live in `revi_investigation_contracts.api` rather than
 * in its `rounds` module, because a turn answer and a portfolio card carry
 ...
 * `TurnAnswer` and `AnomalyCard`, and are re-exported here so a Rounds
...
/**
 * `RoundsTileIntegrity` — the M22 integrity line as a payload.
 *
...
/**
 * `RoundsDeltaPayload` — this tile's movement, in the metric's own unit.
 *
...
export interface RoundsDelta {
  ...
  thresholdSource: "governed" | "watch";
...
export function mapRoundsDelta(raw: unknown): RoundsDelta | undefined {
  if (!isRecord(raw)) return undefined;
  const direction = typeof raw.direction === "string" && DIRECTIONS.has(raw.direction)
    ? (raw.direction as RoundsDelta["direction"])
    : "unknown";
  return {
...
/** `RoundsPinPayload` — one pinned spec, as stored. */
...
/** `RoundsTilePayload` — one pin, evaluated at one load. */
export interface RoundsTile {
  pinId: string;
  label: string;
  presentation: RoundsPresentation;
  ...
  unit?: string;
  metricId?: string;
  /** Never optional. A tile without it is not built — see `mapRoundsTile`. */
  integrity: TileIntegrity;
  warnings: Omit<WarningEvent, "type">[];
  findings: Finding[];
  delta?: RoundsDelta;
  /** Movement since the watch's own creation-load baseline. */
  baselineDelta?: RoundsDelta;
 ...
 */
export function mapRoundsTile(raw: unknown, index: number, drift: string[]): RoundsTile | null {
  if (!isRecord(raw)) {
  ...
  return {
    ...
    status: typeof raw.status === "string" && TILE_STATUSES.has(raw.status)
      ? (raw.status as RoundsTile["status"])
      : "ok",
    ...
    findings,
    ...(mapRoundsDelta(raw.delta) !== undefined ? { delta: mapRoundsDelta(raw.delta) } : {}),
    ...(mapRoundsDelta(raw.baseline_delta) !== undefined
      ? { baselineDelta: mapRoundsDelta(raw.baseline_delta) }
      : {}),
...
export interface RoundsParse {
  value: RoundsData | null;
  drift: string[];
}

export function parseRounds(raw: unknown): RoundsParse {
  const drift: string[] = [];
  if (!isRecord(raw) || !Array.isArray(raw.tiles)) return { value: null, drift: ["tiles"] };
  const tiles: RoundsTile[] = [];
  raw.tiles.forEach((tile, index) => {
    const mapped = mapRoundsTile(tile, index, drift);
...
/** `RoundsMaterialityPayload` — the governed gate that was applied. */
...
/** `RoundsImmaterialSummary` — everything the gate held back, counted. */
...
/** `RoundsFatigueAdvisory` — the brief noticing somebody's own gate is loose. */
export interface BriefFatigue {
  active: boolean;
  watchesBelowGovernedGate: number;
  ...
  return {
    value: {
      ...
      fatigue: {
        ...
        watchesBelowGovernedGate: asNumber(fatigueRaw.watches_below_governed_gate) ?? 0,
...
/** `RoundsLeadPayload` — one lead's lifecycle state. */
