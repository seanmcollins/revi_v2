/**
 * The watch vocabulary exists in FOUR places, and this is the one that
 * asserts they are the same list.
 *
 * Round-8 pinned the engine's `WATCH_THRESHOLD_UNITS` against the wire's
 * `RoundsWatchUnit` in Python CI, because one token of skew (`days`) had
 * 500'd `GET /v1/rounds/pins` for a whole tenant. Nobody pinned the third
 * and fourth copies, both of which live here: `contract.ts`'s `WATCH_UNITS`
 ...
 * actually edits a watch through.
 *
 * The cost of that gap, live, on the watch the VC signed a
 ...
 * refused, so the watch was uneditable and every attempt restarted its
 * baseline.
 *
...
import { WATCH_UNITS } from "@/lib/contract";
...
function engineUnits(): string[] {
  ...
  const match = /WATCH_THRESHOLD_UNITS:\s*tuple\[str, \.\.\.\]\s*=\s*\(([^)]*)\)/.exec(source);
  if (!match) throw new Error("WATCH_THRESHOLD_UNITS not found in ports.py");
...
function wireUnits(): string[] {
  ...
  const unit = spec.components.schemas.RoundsWatchModel?.properties?.unit;
  const branch = unit?.anyOf?.find((b) => Array.isArray(b.enum));
  if (!branch?.enum) throw new Error("RoundsWatchModel.unit enum not found in openapi.json");
  return branch.enum;
}

/** The units the settings control actually offers, off its own markup. */
function offeredUnits(): string[] {
  const source = read("apps/web/src/components/rounds/WatchSensitivity.tsx");
  const select = /aria-label="The unit that number is stated in"[\s\S]*?<\/select>/.exec(source);
  if (!select) throw new Error("the unit <select> was not found in WatchSensitivity.tsx");
  return [...select[0].matchAll(/<option value="([a-z_]+)"/g)].map((m) => m[1]);
}

describe("the watch-unit enum, in every copy of it", () => {
  it("is the same set in contract.ts as in the engine's own module", () => {
    expect([...WATCH_UNITS].sort()).toEqual(engineUnits().sort());
  });

  it("is the same set in contract.ts as on the published wire", () => {
    expect([...WATCH_UNITS].sort()).toEqual(wireUnits().sort());
  });

  it("is offered, one option per unit, by the control that edits a watch", () => {
    ...
    expect(offered.slice().sort()).toEqual([...WATCH_UNITS].sort());
  });
  ...
  it("carries `days` — named, so the regression reads in a failure log", () => {
    ...
    expect(WATCH_UNITS.has("days")).toBe(true);
