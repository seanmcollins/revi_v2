...
import type { BriefEntry, LeadStatus } from "@/lib/monitors";
  ...
  return (
    <li data-brief-entry={entry.kind} className="group relative">
          ...
          <TooltipTrigger asChild>
            ...
            >
              {entry.provenance.source === "detection_feed"
                ? "detected this load"
                : "your monitor, re-run at this load"}
            </button>
          </TooltipTrigger>
      ...
      {/* WHERE THIS LEAD STANDS, on the row that reports it. The control
          ...
          surfaces — so a lead claimed from Monitors is claimed everywhere. */}
      {entry.anomalyId && (
        <LeadStatusControl
...
const KINDS: Readonly<Record<string, { label: string; mark: string; tone: string }>> = {
  ...
  pin_movement: { label: "A monitor moved", mark: "bg-foreground/40", tone: "text-foreground/80" },
  self_resolved: {
  ...
  // ranked monitor headlines is a different cell from last load's. "Your
