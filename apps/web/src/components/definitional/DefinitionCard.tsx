  ...
  return (
    <div className="overflow-hidden rounded-lg border bg-card">
      <div className="flex items-start justify-between gap-3 border-b bg-surface-sunken/50 px-4 py-3">
        ...
        <span className="rounded-full border px-2 py-0.5 font-mono text-[0.62rem] text-muted-foreground">
          {definition.packVersion.packId}@{definition.packVersion.version}
        </span>
      </div>

      <div className="space-y-3 px-4 py-3.5">
        <p className="text-[0.8rem] leading-relaxed">{definition.definition}</p>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {definition.groupCode && (
              <div className="rounded-md border bg-surface-sunken/50 p-2.5">
                <p className="font-mono text-[0.65rem] font-semibold text-verified">
          ...
          <div>
            ...
            <ul className="space-y-0.5">
              {definition.sources.map((source) => (
                <li key={source.label} className="flex items-center gap-1.5 text-[0.7rem]">
                  <span className="rounded-full border px-1.5 py-px text-[0.58rem] uppercase tracking-wide text-muted-foreground">
          ...
          <div>
            ...
            <div className="flex max-w-56 flex-wrap gap-1">
              {definition.relatedConcepts.map((c) => (
                <span
                  key={c}
                  className="rounded-full bg-surface-sunken px-2 py-0.5 text-[0.65rem] text-secondary-foreground"
                >
                  {c}
                </span>
              ))}
