...
class InProcessInvestigationClient:
    ...
    async def open_session(self, request: OpenSessionRequest) -> SessionResponse:
        return await self._service.open_session(request)

    async def submit_turn(self, session_id: str, request: TurnRequest) -> TurnResult:
        return await self._service.submit_turn(session_id, request)

    async def get_investigation(self, investigation_id: str) -> InvestigationResponse:
        return await self._service.get_investigation(investigation_id)

    async def get_session_lineage(self, session_id: str) -> SessionLineageResponse:
        return await self._service.get_session_lineage(session_id)
...
class HttpInvestigationClient:
    ...
    async def open_session(self, request: OpenSessionRequest) -> SessionResponse:
        response = await self._client.post("/v1/sessions", json=request.model_dump(mode="json"))
    ...
    async def get_investigation(self, investigation_id: str) -> InvestigationResponse:
        response = await self._client.get(f"/v1/investigations/{investigation_id}")
        response.raise_for_status()
        return InvestigationResponse.model_validate(response.json())

    async def get_session_lineage(self, session_id: str) -> SessionLineageResponse:
        response = await self._client.get(f"/v1/sessions/{session_id}/lineage")
        response.raise_for_status()
    ...
    async def get_portfolio(self) -> PortfolioResponse:
        response = await self._client.get("/v1/portfolio/latest")
        response.raise_for_status()
