...
**PIN = MONITOR.** A pin stores the ``TypedInvestigationSpec`` behind an
...
adversarial monitors went into those rules. A tile is an answer, so a tile
...
**PER-LOAD EVALUATION.** :meth:`MonitorsService.evaluate_load` is idempotent
per (pin, watermark) and is called from two places for the same reason the
cohort sweep is: the scheduled tick (``revi_api.monitors_sweep``) keeps an
...
(``packs/base-rcm/monitors.yaml`` via :mod:`revi_api.monitors_policy`); this
...
class MonitorsService:
    ...
    async def register_intent_pin(
        ...
        *,
        ...
        is not always what the platform resolved: "monitor Silverline Health"
        resolved to Silverline Medicare Advantage and kept the wrong payer's
        name on the tile permanently; "monitor denied dollars for Meridian HMO
        Care" named a payer this warehouse does not contain; and "monitor
        this" produced a monitor whose entire label was the pronoun. A monitor
        titled after an unresolved phrase is a monitor nobody can check. The
        ...
        so a monitor whose sensitivity the wire could not describe — one
        ...
        briefing, and a confirmed belief that nothing was monitoring. Nothing
        ...
        label = self._composed_label(spec)
        # Already monitoring exactly this? The declaration path bypassed the
        # spec-hash dedupe the on-screen pin path has run since round 7, so
        # two identical monitors over one spec briefed one movement twice
        ...
        if baseline is not None:
            try:
                ...
                # right for the only case it was written for — a monitor minted
                # fresh has no history — and wrong for the case the demo
                # curation creates: a re-minted monitor whose history was
    ...
    async def list_pins(self, principal: Principal) -> MonitorsPinListResponse:
        ...
        monitor the wire could not describe — one ``days`` threshold, legal in
        ...
        A monitor that cannot be fully described is now described as far as it
        can be, with the reason on the row: the analyst can still see WHAT
        is monitored, still un-pin it, and still read why its threshold is not
    ...
    async def repair_pins(self, tenant: str) -> dict[str, list[str]]:
        """Bring monitors created before round 7 onto the narrowed-cell rule.

        Every monitor pinned from one cell of a ranked breakdown stored the
        ...
        * it does not — the monitor is archived (softly, like every other
        ...
        renders the stored TILE: two monitors were narrowed to Pinnacle
        Health Plan, relabelled and had their baselines cleared, and
        ``GET /v1/monitors`` went on serving their pre-repair tiles — "Pinnacle
    ...
    def _spec_summary(self, spec: TypedInvestigationSpec, window_mode: str) -> str:
        ...
        The panel headed "What this monitor measures" rendered a window note
        ...
        control that lets somebody catch a monitor that is measuring the wrong
    ...
    async def evaluate_load(
        ...
        row for this watermark won, so a monitor repaired between loads
        ...
        while it is still an evaluation OF THIS MONITOR — see
    ...
    async def _stale_prior_reason(
        ...
        one asks whether a stored tile still describes THIS MONITOR; this one
        asks whether it still describes the right PAIR OF LOADS. The two
        surfaces disagreed on ``pin_b616b7b89bde`` — the JOC monitor the demo
    ...
    def _delta(
        ...
        nothing, and the renderer draws nothing for nothing — so a monitor
        that had never been compared looked exactly like a monitor that had
    ...
    async def brief_at(
        ...
        *,
        ...
        # against the load the caller named and the monitor movements diffed
    ...
    async def _movement_entries(
        ...
        *,
        ...
        of eighteen monitors at one live load were neither briefed nor
    ...
    async def _delta_against(
        ...
        *,
        ...
        default brief and the tile grid are one view rendered twice; a monitor
...
def _stale_result_reason(pin: MonitorsPin, stored: MonitorsPinResult) -> str | None:
    ...
    keyed on ``(pin, watermark)`` and a monitor can change UNDER a watermark:
...
def _subject_mismatch(
    ...
    two loads are two measurements of one thing. A monitor over a ranked
    ...
    late-arriving data settling — adjudication run-out" for a monitor whose
...
def _movement_sentence(
    ...
    noun is "monitor" and never "tile", and the SUBJECT is named — no
...
#: pin movement") directly above rows the UI labels "A MONITOR MOVED", and
...
def _resolution_clause(
    ...
    Round-8 FIX-10. A monitor is titled from the RESOLVED spec, which is the
    ...
    "monitor Silverline Health" became a monitor on Silverline Medicare
