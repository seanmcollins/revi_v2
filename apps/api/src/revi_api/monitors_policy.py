"""Governed Monitors content: materiality, resolution, time-to-impact.
...
MONITORS_FILENAME = "monitors.yaml"
...
class MonitorsPolicy:
    """The whole governed Monitors content, plus where it came from."""
    ...
    @property
    def enabled(self) -> bool:
        """False when the pack ships no Monitors content.
    ...
    def payload(self) -> MonitorsMaterialityPayload:
        """The gate that was applied, for publication on the brief."""
        return MonitorsMaterialityPayload(
            unit_kinds={
...
def load_monitors_policy(path: str | Path) -> MonitorsPolicy:
    """Read the governed Monitors content, or an empty policy when absent."""
    file = Path(path)
    if not file.is_file():
        return MonitorsPolicy()
...
def assess_movement(
    ...
    * the ANALYST's, when the monitor declares its own mode. It may be
    ...
    # monitor that says "tell me when it gets worse" has said something about
    # every threshold beneath it.
    if monitor is not None and monitor.direction != "any" and delta != 0:
        wanted_up = monitor.direction == "up"
        if (delta > 0) != wanted_up:
            return MaterialityVerdict(
                False,
                "monitor_direction",
                f"this monitor is set to movements {monitor.direction} only, and it moved "
    ...
    own = _monitor_verdict(monitor, unit, delta, prior, current, magnitude_text)
    return MaterialityVerdict(
...
def _monitor_verdict(
    ...
    if monitor.mode == "any_movement":
        ...
        return MaterialityVerdict(
            material,
            "monitor_any_movement",
            f"this monitor is set to brief on any movement at all, and it moved "
            f"{magnitude_text}"
            if material
            else "this monitor is set to brief on any movement at all, and it did not move",
        )
    if monitor.value is None:
        return MaterialityVerdict(
            False,
            "monitor_incomplete",
            f"this monitor declares mode {monitor.mode!r} with no threshold value, so it cannot "
            "be evaluated and nothing is briefed",
        )
    if monitor.mode == "crosses":
        level = _threshold_in_metric_unit(monitor, unit, reference=None)
        if level is None:
            return MaterialityVerdict(False, "monitor_unit_mismatch", _unit_mismatch_note(monitor, unit))
        crossed = (prior < level <= current) or (current <= level < prior)
        return MaterialityVerdict(
            crossed,
            "monitor_crosses",
            (
                f"the value crossed the monitored level of {format_threshold(monitor, unit)}: "
                f"{magnitude(prior, unit)} to {magnitude(current, unit)}"
            )
            if crossed
            else (
                f"the value did not cross the monitored level of "
                f"{format_threshold(monitor, unit)} at this load"
            ),
        )
    # delta_gte
    gate = _threshold_in_metric_unit(monitor, unit, reference=prior)
    if gate is None:
        return MaterialityVerdict(False, "monitor_unit_mismatch", _unit_mismatch_note(monitor, unit))
    ...
    return MaterialityVerdict(
        material,
        "monitor_delta_gte",
        f"this monitor briefs at {format_threshold(monitor, unit)} and it moved "
        f"{magnitude_text}"
        + (f" ({monitor.note})" if monitor.note else ""),
    )


def _threshold_in_metric_unit(
    monitor: Monitor, unit: str | None, *, reference: Decimal | None
) -> Decimal | None:
    """The monitor's threshold expressed in the metric's own unit.
    ...
    threshold is a monitor that fires for a reason nobody can see.
    """
    if monitor.value is None:
        return None
    if monitor.unit == "points":
        return monitor.value / 100 if unit == RATIO_UNIT else None
    if monitor.unit == "cents":
        return monitor.value if unit == MONEY_UNIT else None
    if monitor.unit == "days":
        return monitor.value if unit == DAYS_UNIT else None
    if monitor.unit == "relative_pct":
        if reference is None or not reference:
            return None
        return abs(reference) * monitor.value / 100
    # No unit stated: read the threshold in the metric's own unit, which is
    # the only reading that cannot be wrong.
    return monitor.value


def _unit_mismatch_note(monitor: Monitor, unit: str | None) -> str:
    return (
        f"this monitor states its threshold in {monitor.unit!r}, which is not an honest unit for "
        f"a {unit or 'unknown'!r} contract, so it cannot be evaluated and nothing is briefed "
        "(the pairing was legal when the monitor was created — the metric's declared unit has "
        "changed since)"
    )
...
def format_threshold(monitor: Monitor, unit: str | None) -> str:
    ...
    if monitor.unit == "points":
        ...
        rendered = f"{float(monitor.value):.2f}".replace(".00", "")
        return f"{rendered} point" + ("" if float(monitor.value) == 1 else "s")
    if monitor.unit == "cents":
        return magnitude(int(monitor.value), MONEY_UNIT)
    if monitor.unit == "days":
        return magnitude(monitor.value, DAYS_UNIT)
    if monitor.unit == "relative_pct":
        return f"{float(monitor.value):.1f}% of the reference value"
    return magnitude(monitor.value, unit)
...
def validate_monitor(monitor: Monitor, *, units: Sequence[str | None]) -> str | None:
    ...
    if monitor.mode not in MONITOR_MODES:
        return (
            f"unknown monitor mode {monitor.mode!r} — the closed set is "
            f"{', '.join(MONITOR_MODES)}"
        )
    if monitor.mode in ("delta_gte", "crosses"):
        if monitor.value is None:
            return f"a {monitor.mode!r} monitor needs a threshold value; none was given"
        if monitor.unit is None:
            return (
                f"a {monitor.mode!r} monitor needs the unit its threshold is stated in "
                f"({', '.join(MONITOR_THRESHOLD_UNITS)}) — an unlabelled number cannot be "
                "compared honestly against a metric whose unit it may not share"
            )
        if monitor.unit not in MONITOR_THRESHOLD_UNITS:
            return (
                f"unknown threshold unit {monitor.unit!r} — the closed set is "
                f"{', '.join(MONITOR_THRESHOLD_UNITS)}"
            )
    elif monitor.value is not None:
        return (
            f"a {monitor.mode!r} monitor takes no threshold value, and one was given "
            f"({monitor.value}) — it would be a number with nothing to compare against"
        )
    if monitor.direction not in ("any", "up", "down"):
        return f"unknown monitor direction {monitor.direction!r} — expected any, up or down"
    if monitor.unit is None or monitor.unit == "relative_pct":
    ...
    if not distinct:
        return (
            f"this monitor states its threshold in {monitor.unit!r}, and the pack declares no unit "
    ...
    if len(distinct) > 1:
        return (
            ...
            f"so a threshold stated in {monitor.unit!r} has no single meaning over it — state it "
            "as relative_pct, or monitor one metric at a time"
        )
    unit = next(iter(distinct))
    expected = {"points": RATIO_UNIT, "cents": MONEY_UNIT, "days": DAYS_UNIT}[monitor.unit]
    if unit != expected:
        advice = {
            "points": (
        ...
        return (
            f"a threshold in {monitor.unit!r} is only honest for a {expected!r} contract, and "
            f"this monitor measures {unit!r}. " + advice
        )
    return None
...
def time_to_impact_for(
    ...
    if rule is None:
        return TimeToImpactPayload(
            ...
            reason=f"category {record.category!r} has no rule in the governed Monitors content",
        )
