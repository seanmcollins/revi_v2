"""Rounds evaluation on watermark advance, inside the API process.
...
tenant's Rounds at the new load: every active watch re-run, every claimed
resolution verified, the detection-feed census recorded. The brief route
calls the *same* :meth:`~revi_api.rounds.RoundsService.evaluate_load`
...
* **Switch-offable.** ``REVI_ROUNDS_SWEEP_INTERVAL_SECONDS=0`` disables the
...
* **Tenant-scoped by the store, not by a list here.** The pin store answers
  "which tenants hold an active watch", so a tenant with nothing pinned
...
logger = logging.getLogger("revi.api.rounds_sweep")

SWEEP_INTERVAL_ENV = "REVI_ROUNDS_SWEEP_INTERVAL_SECONDS"
...
@runtime_checkable
class RoundsEvaluator(Protocol):
    """The slice of the Rounds service this loop needs.

    Structural on purpose, exactly like ``CohortSweeper``: this module
    names no service class, so a deployment that wires a different Rounds
...
def sweep_interval_seconds(env: Mapping[str, str]) -> float:
    """``REVI_ROUNDS_SWEEP_INTERVAL_SECONDS``; <= 0 disables the loop.
    ...
    try:
        ...
        logger.error(
            "%s=%r is not a number — falling back to %.0fs rather than leaving Rounds "
            "un-evaluated",
            SWEEP_INTERVAL_ENV,
            raw,
...
async def evaluate_once(
    ...
    *,
    ...
    """Evaluate every tenant's Rounds at the newest load. Never raises.
    ...
    """
    if not isinstance(rounds, RoundsEvaluator) or not isinstance(pins, TenantSource):
        logger.warning(
            "Rounds evaluation is NOT running in this process: the wired components expose "
            "no evaluator (%s) or no pin store (%s)",
            type(rounds).__name__,
            type(pins).__name__,
        )
        return last_watermark_id
    if not isinstance(watermarks, WatermarkSource):  # pragma: no cover - defensive
        logger.warning("no watermark source is wired — Rounds evaluation is off")
        return last_watermark_id
    try:
        newest = await watermarks.newest_watermark()
    except Exception:
        logger.warning("could not read the newest watermark — Rounds evaluation deferred")
        return last_watermark_id
    if last_watermark_id == newest.id:
        logger.debug("rounds: watermark %s has not advanced; nothing to evaluate", newest.id)
        return last_watermark_id
    try:
        tenants = await pins.tenants_with_pins()
    except Exception:
        logger.exception("rounds: could not list tenants with watches — the next tick retries")
        return last_watermark_id
    if not tenants:
        logger.info(
            "rounds: watermark advanced to %s and no tenant holds a watch — nothing to walk",
            newest.id,
        )
        return newest.id
    evaluated = 0
    for tenant in tenants:
        try:
            await rounds.evaluate_load(tenant, newest)
            evaluated += 1
        except Exception:
            # Deliberately broad and deliberately per-tenant: one tenant's
            # broken watch must not cost every other tenant their brief.
            logger.exception(
                "rounds: evaluation failed for tenant %s at %s — the next tick retries",
                tenant,
                newest.id,
            )
    logger.info(
        "rounds: walked %d of %d tenant(s) at watermark %s", evaluated, len(tenants), newest.id
    )
...
class RoundsSweepScheduler:
    ...
    async def start(self) -> str | None:
        ...
        if not self.enabled:
            logger.warning(
                "%s=%s — periodic Rounds evaluation is DISABLED in this process; briefs are "
            ...
            return None
        logger.info(
            "Rounds evaluation enabled: startup walk + a watermark check every %.0fs",
            self._interval,
        )
        self._last = await evaluate_once(
            self._rounds, self._pins, self._watermarks, last_watermark_id=self._last
        )
        self._task = asyncio.create_task(self._loop(), name="revi-rounds-sweep")
        return self._last
