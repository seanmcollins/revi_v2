...
from revi_api.monitor_intent import legal_threshold_phrases, parse_monitor_declaration
from revi_investigation.application.ports import MONITOR_THRESHOLD_UNITS
...
class TestRecognition:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # registering a monitor over an empty subject would be the
            # platform inventing what to monitor.
            "watch",
            "monitor",
            "keep an eye on",
        ],
    ...
    def test_ordinary_language_is_left_alone(self, utterance: str) -> None:
        assert parse_monitor_declaration(utterance) is None

    def test_the_lead_in_is_echoed_verbatim(self) -> None:
        """The platform shows what it READ rather than asserting it read
        intent."""
        declaration = parse_monitor_declaration("Keep an eye on the denial rate")
        assert declaration is not None
    ...
    def test_filler_between_the_lead_in_and_the_subject_is_stripped(self) -> None:
        with_the = parse_monitor_declaration("watch the denial rate for Silverline")
        without = parse_monitor_declaration("watch denial rate for Silverline")
...
class TestSubjectIsolation:
    def test_a_threshold_clause_never_reaches_the_subject(self) -> None:
        ...
        interpreted as a question about a movement, and the monitor would be
        defined by that spec forever."""
        declaration = parse_monitor_declaration(
    ...
    def test_scope_specificity_is_just_the_subject(self) -> None:
        ...
        broad = parse_monitor_declaration("watch our denial rate")
        narrow = parse_monitor_declaration(
...
class TestStatedSensitivity:
    def test_points_stay_points(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.mode == "delta_gte"
        assert declaration.monitor.value == Decimal("2")
        assert declaration.monitor.unit == "points"
        assert declaration.monitor.direction == "up"
    ...
    def test_dollars_are_carried_in_cents(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.unit == "cents"
        assert declaration.monitor.value == Decimal("5000000")
    ...
    def test_a_bare_percent_reads_as_relative_never_as_points(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.unit == "relative_pct"
    ...
    def test_a_crossing_is_a_level_not_a_relative_amount(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.mode == "crosses"
        assert declaration.monitor.unit == "points"
        assert declaration.monitor.value == Decimal("15")
    ...
    def test_any_movement_is_recognised(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.mode == "any_movement"
    ...
    def test_a_direction_with_no_size_narrows_the_governed_default(self) -> None:
        ...
        declaration = parse_monitor_declaration("watch denial rate, tell me if it rises")
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.mode == "governed_default"
        assert declaration.monitor.direction == "up"

    def test_an_unrecognised_clause_falls_back_to_the_governed_gate(self) -> None:
        """Never a guessed threshold: a monitor that fires on a number nobody
        stated is worse than one that fires on the pack's."""
        declaration = parse_monitor_declaration(
            "watch denial rate, tell me when something interesting happens"
        )
        assert declaration is not None
        assert declaration.monitor is None or declaration.monitor.mode == "governed_default"

    def test_the_analysts_own_clause_is_recorded_on_the_monitor(self) -> None:
        declaration = parse_monitor_declaration(
            "watch denial rate, alert me if it rises more than 2 points"
        )
        assert declaration is not None and declaration.monitor is not None
        assert "2 points" in declaration.monitor.note
...
class TestAStatedSensitivityIsNeverSilentlyReplaced:
    ...
    def test_the_shapes_people_actually_type_are_read(
        ...
        declaration = parse_monitor_declaration(utterance)
        assert declaration is not None and declaration.monitor is not None, utterance
        assert declaration.monitor.value == Decimal(value)
        assert declaration.monitor.unit == unit
    ...
    def test_a_days_threshold_is_read_and_is_a_legal_stated_unit(self) -> None:
        ...
        assert declaration is not None and declaration.monitor is not None
        assert declaration.monitor.unit == "days"
        assert declaration.monitor.unit in MONITOR_THRESHOLD_UNITS
    ...
    def test_no_monitor_registers_the_governed_default_over_a_stated_number(self) -> None:
        ...
        for utterance in (
            ...
            assert declaration is not None
            silently_defaulted = (
                declaration.monitor is not None
                and declaration.monitor.mode == "governed_default"
