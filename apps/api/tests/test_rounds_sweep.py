"""The Rounds walk: watermark-triggered, never fatal, switch-offable.
...
* evaluation happens on watermark ADVANCE, not on every tick — re-running
  every watch against data that has not changed is how a background loop
  becomes expensive enough to switch off;
* one tenant's broken watch must not cost every other tenant their brief;
* a typo in the interval falls back LOUDLY rather than silently turning the
  proactive half of the product off;
* a process wired with no Rounds service says so and does nothing, instead
...
from revi_api.rounds_sweep import (
    DEFAULT_SWEEP_INTERVAL_SECONDS,
    RoundsSweepScheduler,
    evaluate_once,
...
class _Rounds:
    ...
    async def evaluate_load(
        ...
        if tenant in self._failing:
            raise RuntimeError("this tenant's watch is broken")
        return None
...
class TestInterval:
    ...
    def test_zero_disables_the_loop(self) -> None:
        assert sweep_interval_seconds({"REVI_ROUNDS_SWEEP_INTERVAL_SECONDS": "0"}) == 0
    ...
    def test_a_typo_falls_back_loudly_rather_than_disabling(
        ...
        with caplog.at_level("ERROR"):
            value = sweep_interval_seconds({"REVI_ROUNDS_SWEEP_INTERVAL_SECONDS": "fifteen"})
        assert value == DEFAULT_SWEEP_INTERVAL_SECONDS
        assert "is not a number" in caplog.text


class TestEvaluateOnce:
    async def test_it_walks_every_tenant_holding_a_watch(self) -> None:
        rounds = _Rounds()
        reached = await evaluate_once(rounds, _Pins(("a", "b")), _Watermarks(WM_1))
        assert reached == "wm_001"
        assert rounds.calls == [("a", "wm_001"), ("b", "wm_001")]
    ...
    async def test_an_unadvanced_watermark_evaluates_nothing(self) -> None:
        ...
        reached = await evaluate_once(
            rounds, _Pins(("a",)), _Watermarks(WM_1), last_watermark_id="wm_001"
        )
        assert reached == "wm_001"
        assert rounds.calls == []
    ...
    async def test_an_advance_evaluates_again(self) -> None:
        rounds = _Rounds()
        reached = await evaluate_once(
            rounds, _Pins(("a",)), _Watermarks(WM_2), last_watermark_id="wm_001"
        )
        assert reached == "wm_002"
        assert rounds.calls == [("a", "wm_002")]

    async def test_one_broken_tenant_does_not_cost_the_others(self) -> None:
        rounds = _Rounds(failing=frozenset({"a"}))
        reached = await evaluate_once(rounds, _Pins(("a", "b")), _Watermarks(WM_1))
        assert reached == "wm_001"
        assert ("b", "wm_001") in rounds.calls

    async def test_no_tenant_with_a_watch_is_a_no_op(self) -> None:
        rounds = _Rounds()
        reached = await evaluate_once(rounds, _Pins(()), _Watermarks(WM_1))
        assert reached == "wm_001"
        assert rounds.calls == []
...
class TestScheduler:
    async def test_a_disabled_scheduler_evaluates_nothing_and_warns(
        ...
        rounds = _Rounds()
        scheduler = RoundsSweepScheduler(
            rounds, _Pins(("a",)), _Watermarks(WM_1), interval_seconds=0
        )
        with caplog.at_level("WARNING"):
    ...
    async def test_startup_walks_the_current_load_once(self) -> None:
        ...
        rounds = _Rounds()
        scheduler = RoundsSweepScheduler(
            rounds, _Pins(("a",)), _Watermarks(WM_1), interval_seconds=3600
        )
        try:
            assert await scheduler.start() == "wm_001"
            assert rounds.calls == [("a", "wm_001")]
