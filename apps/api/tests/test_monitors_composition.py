...
from revi_api.monitors_policy import MonitorsPolicy, load_monitors_policy
...
PACK_MONITORS = "packs/base-rcm/monitors.yaml"
...
def _pin(spec: TypedInvestigationSpec, label: str = "monitored") -> MonitorsPin:
    return MonitorsPin(
        id="pin_test",
        tenant="demo",
        label=label,
...
def _tile(**overrides: object) -> MonitorsTilePayload:
    base: dict[str, object] = {
        "pin_id": "pin_test",
        "label": "monitored",
        "presentation": "finding",
    ...
    base.update(overrides)
    return MonitorsTilePayload(**base)  # type: ignore[arg-type]
...
def _entry(kind: str, *, title: str = "", impact: int | None = None) -> MonitorsBriefEntry:
    return MonitorsBriefEntry(
        kind=kind,  # type: ignore[arg-type]
        title=title or kind,
        statement="",
        impact_cents=impact,
        provenance=MonitorsProvenancePayload(source="detection_feed"),
    )
...
class TestDuplicateSpecs:
    def test_order_does_not_make_two_monitors_out_of_one(self) -> None:
    ...
    def test_a_different_cell_is_a_different_monitor(self) -> None:
...
class TestTheCapDropsByConsequence:
    @staticmethod
    def _policy() -> MonitorsPolicy:
        return load_monitors_policy(PACK_MONITORS)
...
class TestBriefCopy:
    def test_the_headline_speaks_dates_and_nouns(self) -> None:
        ...
        assert sentence == (
            "Since the Aug 1 load: 2 new leads, 1 monitor moved, 1 resolved on its own."
        )
    ...
    def test_the_proud_shape_stays_proud_and_stays_human(self) -> None:
        ...
        assert "1 monitor " in sentence and "33 detected leads" in sentence
    ...
    def test_the_held_back_line_counts_every_bucket(self) -> None:
        ...
        assert "1 monitor moved by less than" in note
        assert "16 monitors have nothing to compare against yet" in note
        assert "2 monitors could not be measured" in note
...
def test_a_delta_payload_can_state_a_first_reading() -> None:
    ...
    nothing — so a monitor that had never been compared looked exactly like
    one that had not moved. Absence is read as absence only if something
    says so."""
    payload = MonitorsDeltaPayload(
        comparable=False,
