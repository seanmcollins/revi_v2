...
``GET /v1/rounds/leads/ANM-029`` explained it: ``claimed_at_watermark
...
from revi_investigation.application.ports import RoundsLead, RoundsLoad
...
from revi_investigation_contracts.rounds import RoundsLeadPatchRequest
...
def _lead(**overrides: object) -> RoundsLead:
    ...
    return RoundsLead(**base)  # type: ignore[arg-type]
...
class TestVerdictsSurviveARewalk:
    ...
    def _load(self, verifications: list[dict[str, object]]) -> RoundsLoad:
        return RoundsLoad(
            tenant=TENANT,
            watermark_id="wm_010",
...
class TestOverRealLoads:
    ...
    async def test_loads_before_the_claim_never_verify_it(self) -> None:
        ...
        claimed = await service.rounds.patch_lead(
            CALLER,
            "ANM-029",
            RoundsLeadPatchRequest(status="resolved_claimed", note="rebilled the 14 accounts"),
            watermark=third,
        )
        ...
        lead = await service.rounds.get_lead(CALLER, "ANM-029")
    ...
    async def test_a_confirmed_lead_that_reappears_regresses(self, tmp_path: Path) -> None:
        ...
        await service.rounds.brief_at(CALLER, third)
        assert (await service.rounds.get_lead(CALLER, "ANM-031")).status == "resolved_confirmed"

        brief = await service.rounds.brief_at(CALLER, fourth)
        ...
        again = await service.rounds.brief_at(CALLER, fourth)
        assert [e.anomaly_id for e in again.entries if e.kind == "resolution_regressed"] == [
            "ANM-031"
        ]
        assert (await service.rounds.get_lead(CALLER, "ANM-031")).verification_note == (
