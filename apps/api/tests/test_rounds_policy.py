"""The governed Rounds gate: materiality, watch thresholds, cash timing.

Alert fatigue is the death mode of a daily surface, and every number that
decides whether somebody is interrupted lives in ``packs/base-rcm/rounds.yaml``
...
from revi_investigation.application.ports import AnomalyRecord, RoundsWatch

REPO_ROOT = Path(__file__).resolve().parents[3]
PACK = REPO_ROOT / "packs" / "base-rcm" / ROUNDS_FILENAME


@pytest.fixture(scope="module")
def policy():  # type: ignore[no-untyped-def]
    return load_rounds_policy(PACK)
...
class TestTheGovernedFileLoads:
    def test_the_pack_ships_rounds_content(self, policy) -> None:  # type: ignore[no-untyped-def]
        assert policy.enabled
        assert policy.content_hash
        assert policy.source.endswith(ROUNDS_FILENAME)

    def test_a_missing_file_is_an_absence_not_an_error(self, tmp_path: Path) -> None:
        """A deployment whose pack ships no Rounds content still pins and
        still evaluates; it simply has no gate, and the brief says so."""
        empty = load_rounds_policy(tmp_path / "nope.yaml")
    ...
    def test_a_time_to_impact_refusal_must_state_its_reason(self, tmp_path: Path) -> None:
        ...
        bad = tmp_path / "rounds.yaml"
        bad.write_text(
            "time_to_impact:\n  categories:\n    mystery:\n      method: unknown\n",
            encoding="utf-8",
        )
        with pytest.raises(ValueError, match="without a reason"):
            load_rounds_policy(bad)
...
class TestWatchThresholds:
    def test_any_movement_briefs_a_move_the_governed_gate_would_not(
        ...
        verdict = assess_movement(
            ...
            policy=policy.materiality,
            watch=RoundsWatch(mode="any_movement"),
        )
        assert verdict.material
        assert verdict.threshold_source == "watch"
        assert verdict.below_governed_gate, "the fatigue counter would never fire"

    def test_a_tighter_watch_holds_a_move_the_governed_gate_would_brief(
        self, policy
    ) -> None:  # type: ignore[no-untyped-def]
        verdict = assess_movement(
            unit="ratio",
            ...
            current=Decimal("0.112"),
            policy=policy.materiality,
            watch=RoundsWatch(mode="delta_gte", value=Decimal(5), unit="points"),
        )
        assert not verdict.material
        assert verdict.threshold_source == "watch"
    ...
    def test_direction_applies_to_every_mode(self, policy) -> None:  # type: ignore[no-untyped-def]
        verdict = assess_movement(
            ...
            policy=policy.materiality,
            watch=RoundsWatch(mode="any_movement", direction="up"),
        )
        assert not verdict.material and verdict.rule == "watch_direction"

    def test_crosses_fires_only_on_the_load_that_crosses(self, policy) -> None:  # type: ignore[no-untyped-def]
        watch = RoundsWatch(mode="crosses", value=Decimal(15), unit="points")
        ...
        assert crossing.material and crossing.rule == "watch_crosses"
    ...
    def test_a_dishonest_unit_pairing_is_refused_at_creation(self) -> None:
        assert validate_watch(
            RoundsWatch(mode="delta_gte", value=Decimal(1), unit="points"),
            units=["money_cents"],
        )
        assert validate_watch(
            RoundsWatch(mode="delta_gte", value=Decimal(1), unit="cents"),
            units=["ratio"],
        )
        assert (
            validate_watch(
    ...
    def test_relative_pct_is_legal_against_any_unit(self) -> None:
        ...
        for units in (["ratio"], ["money_cents"], ["days"], ["money_cents", "ratio"]):
            assert (
                validate_watch(
                    RoundsWatch(mode="delta_gte", value=Decimal(10), unit="relative_pct"),
                    units=units,
                )
                is None
            )
    ...
    def test_a_valueless_mode_refuses_a_value(self) -> None:
        refusal = validate_watch(
            RoundsWatch(mode="any_movement", value=Decimal(1)), units=["ratio"]
        )
