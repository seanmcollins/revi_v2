...
#: Re-exported so a Monitors client imports one module. These four shapes
...
MonitorsPresentation = Literal["chart", "finding", "worklist_slice", "scalar"]
...
#: :class:`~revi_investigation.application.ports.MonitorsPin`.
MonitorsWindowMode = Literal["relative", "absolute", "anchored"]


class CreateMonitorsPinRequest(ClosedModel):
    """Add a monitor to Monitors.
    ...
    * an utterance — "monitor Silverline's denial rate" — which is not this
    ...
    presentation: MonitorsPresentation = "finding"
...
class MonitorsPinPayload(ClosedModel):
    ...
    #: load evaluates it: a monitor created between loads has no baseline
    #: yet, and taking the previous load's value would attribute a movement
    #: to a period nobody was monitoring.
...
class MonitorsPinListResponse(ClosedModel):
    tenant: str = ""
    pins: list[MonitorsPinPayload] = Field(default_factory=list)
    ...
    #: monitor that vanishes from the list without a word is a monitor somebody
...
class MonitorsDeltaPayload(ClosedModel):
    ...
    #: What this delta is measured FROM: the prior load, or the monitor's own
    #: creation-load baseline. Both are published when they differ
    #: materially — see :class:`MonitorModel` for why.
    reference: Literal["prior_load", "baseline"] = "prior_load"
    #: WHICH CELL each side measured, in the reader's words. A monitor over a
...
class MonitorsTilePayload(ClosedModel):
    ...
    #: The dates this evaluation actually measured, after the monitor's window
    ...
    #: :attr:`MonitorsPinPayload.window_mode` alone.
    ...
    #: The honesty marks. Never optional — see :class:`MonitorsTileIntegrity`.
    integrity: MonitorsTileIntegrity = Field(default_factory=MonitorsTileIntegrity)
    ...
    delta: MonitorsDeltaPayload | None = None
    #: Movement since the monitor's CREATION-LOAD baseline, published when a
    #: baseline exists and it says something the prior-load delta does not.
    #: See :class:`MonitorModel` for the semantics.
    baseline_delta: MonitorsDeltaPayload | None = None
    #: Why the tile has no value, when it has none — in the platform's own
    #: error vocabulary (``GRAIN_INCOMPATIBLE: ...``), never silence.
    unavailable_reason: str | None = None


class MonitorsResponse(ClosedModel):
    """The Monitors surface at one load: every active pin, evaluated."""

    tenant: str = ""
    watermark_id: str = ""
    newest_data_date: date | None = None
    prior_watermark_id: str | None = None
    tiles: list[MonitorsTilePayload] = Field(default_factory=list)
    warnings: list[str] = Field(default_factory=list)
    warnings_v2: list[WarningPayload] = Field(default_factory=list)
...
class MonitorsLeadPayload(ClosedModel):
...
class MonitorsLeadPatchRequest(ClosedModel):
...
class MonitorsProvenancePayload(ClosedModel):
...
class MonitorsBriefEntry(ClosedModel):
    ...
    * ``rank_flip`` — the cell a ranked monitor headlines is not the cell it
    ...
    delta: MonitorsDeltaPayload | None = None
    #: The same movement measured from the monitor's creation-load baseline,
    ...
    integrity: MonitorsTileIntegrity | None = None
    provenance: MonitorsProvenancePayload


class MonitorsMaterialityPayload(ClosedModel):
...
class MonitorsImmaterialSummary(ClosedModel):
    ...
    #: Monitors that were evaluated and have nothing to compare against at
    #: the load this brief diffs from — a first reading, or a monitor created
...
class MonitorsFatigueAdvisory(ClosedModel):
    ...
    active: bool = False
    #: Monitors whose own threshold briefed a movement the governed gate
    #: calls normal variation, at THIS load.
    monitors_below_governed_gate: int = 0
...
class MonitorsBriefResponse(ClosedModel):
    ...
    * ``material_changes`` — entries follow.
    ...
    headline: str = ""
    entries: list[MonitorsBriefEntry] = Field(default_factory=list)
    ...
    entries_total: int = 0
    immaterial: MonitorsImmaterialSummary = Field(default_factory=MonitorsImmaterialSummary)
    #: The brief telling somebody their own thresholds are too loose —
    #: once, when it has been true for long enough to be a pattern.
    fatigue: MonitorsFatigueAdvisory = Field(default_factory=MonitorsFatigueAdvisory)
    materiality: MonitorsMaterialityPayload = Field(default_factory=MonitorsMaterialityPayload)
