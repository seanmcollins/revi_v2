...
* the repair pass rewrote the PIN and the surface renders the TILE, so a
  test that asserted on the pin row passed while two repaired watches went
...
* ``days`` was legal in the engine and missing from the wire's enum, so one
  stored watch 500'd the whole tenant's pin list and a days declaration
...
from revi_api.rounds import _stale_result_reason, _subject_mismatch
...
class TestARepairedWatchChangesWhatTheSurfaceServes:
    """FIX-1. The repair pass narrowed specs, recomposed labels and cleared
    baselines — and ``GET /v1/rounds`` went on serving the tiles those
    watches had before the repair, because evaluation reuses any stored
    ...
    async def test_the_served_tile_is_re_derived_in_the_repair_s_own_pass(
        ...
        # A watch in the shape the store held before round 7: the whole
        ...
        stored_pin = await service.components.rounds_pins.get(created.pin_id)
        ...
        # is about: another cell's number under this watch's old title, with
        ...
        await service.components.rounds_results.put(
            RoundsPinResult(
                pin_id=created.pin_id,
                tenant=TENANT,
                watermark_id=newest.id,
        ...
        after = await service.rounds.rounds_at(CALLER, newest)

        [tile] = [t for t in after.tiles if t.pin_id == created.pin_id]
        repaired = await service.components.rounds_pins.get(created.pin_id)
    ...
    async def test_a_tile_that_no_longer_matches_its_watch_is_not_republished(
        self,
    ) -> None:
        """The belt to the repair's braces: reuse is conditional on the
        stored tile still being an evaluation OF THIS WATCH. Anything that
        ...
        created = await service.rounds.create_pin(
            CALLER,
            CreateRoundsPinRequest(spec=_ranked_spec(), label="denial rate by payer"),
        )
        await service.rounds.rounds_at(CALLER, newest)
        first = await service.components.rounds_results.get(created.pin_id, newest.id)
        assert first is not None
        pin = await service.components.rounds_pins.get(created.pin_id)
        assert pin is not None
        await service.components.rounds_pins.save(replace(pin, label="renamed by its owner"))

        surface = await service.rounds.rounds_at(CALLER, newest)

        [tile] = [t for t in surface.tiles if t.pin_id == created.pin_id]
        assert tile.label == "renamed by its owner"
        second = await service.components.rounds_results.get(created.pin_id, newest.id)
        assert second is not None and second.evaluated_at > first.evaluated_at

    async def test_an_unchanged_watch_still_reuses_its_stored_tile(self) -> None:
        ...
        again = await service.components.rounds_results.get(created.pin_id, newest.id)
    ...
    def test_a_stored_tile_with_no_recorded_subject_is_stale(self) -> None:
        ...
        subjectless = RoundsPinResult(
            pin_id=pin.id,
            tenant=TENANT,
            watermark_id="wm_003",
            watermark_loaded_at=_now(),
            evaluated_at=_now(),
            payload=RoundsTilePayload(
...
class TestARankFlipIsNeverAMovement:
    ...
    async def test_the_brief_publishes_the_flip_and_never_a_movement(self) -> None:
        ...
        await service.rounds.brief_at(CALLER, first)
    ...
    async def test_a_delta_over_unrecorded_subjects_publishes_no_percentage(
        ...
        subjects, on a watch over a ranked breakdown. The numbers stay on
        ...
        for watermark in (first, third):
            stored = await service.components.rounds_results.get(pin.id, watermark.id)
        ...
        delta = await service.rounds._delta_against(
            pin,
            RoundsTilePayload.model_validate(
                (
                    await service.components.rounds_results.get(pin.id, third.id)
...
class TestDaysIsALegalThresholdEverywhere:
    """FIX-3. One token of enum skew: a 500 for the whole tenant, a watch
    ...
    async def test_a_days_watch_survives_a_round_trip_through_the_pin_list(
        self,
    ) -> None:
        service = _service()
        created = await service.rounds.create_pin(
            ...
            CreateRoundsPinRequest(
                ...
                watch=RoundsWatchModel(mode="delta_gte", value=2, unit="days"),
            ),
        )

        listed = await service.rounds.list_pins(CALLER)

        [row] = [p for p in listed.pins if p.pin_id == created.pin_id]
        assert row.watch is not None and row.watch.unit == "days"
        assert listed.unreadable == []
        assert listed.total == len(listed.pins)

    async def test_one_undescribable_watch_cannot_fail_the_whole_tenant_s_list(
        self,
    ) -> None:
        """The failure that actually shipped, generalised: whatever the next
        skew is, it costs its own row and not every other watch on the
        tenant."""
        service = _service()
        good = await service.rounds.create_pin(
            CALLER,
            CreateRoundsPinRequest(spec=_ranked_spec(), label="denial rate by payer"),
        )
        bad = await service.rounds.create_pin(
            CALLER,
            CreateRoundsPinRequest(
                spec=TypedInvestigationSpec(
        ...
        stored = await service.components.rounds_pins.get(bad.pin_id)
        assert stored is not None
        # A unit no version of this wire has ever known.
        await service.components.rounds_pins.save(
            replace(
                stored, watch=RoundsWatch(mode="delta_gte", value=Decimal("2"), unit="furlongs")
            )
        )
        ...
        assert degraded.watch is None
        assert degraded.notes and "could not be described" in degraded.notes[0]
        assert degraded.spec.metric_ids == ["denied_dollars"], (
            "what a watch MEASURES is the part a reader needs most and the part "
            "that never failed"
        )
        assert listed.unreadable == []

    async def test_a_declaration_over_a_watched_spec_does_not_store_a_second_pin(
        self,
    ) -> None:
        ...
        first = await service.rounds.register_intent_pin(
            CALLER, outcome, stated_subject="denial rate", watch=None, matched_phrase="watch"
        )
        second = await service.rounds.register_intent_pin(
            CALLER, outcome, stated_subject="denial rate", watch=None, matched_phrase="watch"
        )

        assert second.pin_id == first.pin_id
        assert "already watching this" in second.statement
        assert (await service.rounds.list_pins(CALLER)).total == 1


class TestAWatchIsTitledByWhatItResolvedTo:
    """FIX-10. A watch clarified off a hallucinated payer kept the wrong
    name permanently, and "watch this" produced a watch labelled ``this``."""
    ...
    async def test_the_label_comes_from_the_spec_and_the_confirmation_says_so(
        ...
        payload = await service.rounds.register_intent_pin(
            CALLER, outcome, stated_subject="this", watch=None, matched_phrase="watch this"
        )
        ...
        stored = await service.components.rounds_pins.get(payload.pin_id)
