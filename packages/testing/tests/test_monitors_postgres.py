...
TENANT = "demo"
CALLER = Principal(tenant=TENANT, subject="monitors-postgres-suite")
    ...
    async def test_the_whole_surface_survives_the_database(
        ...
        pin = await service.monitors.create_pin(
            ...
            CreateMonitorsPinRequest(
                ...
                monitor=MonitorModel(mode="delta_gte", value=5.0, unit="relative_pct"),
            ),
        )
        # The threshold is a Decimal in the store and comes back a Decimal:
        # a monitor is a promise about a specific number, and a threshold that
        ...
        assert isinstance(stored.monitor.value, Decimal)
        assert stored.monitor.value == Decimal("5")

        # ---- load 1: the baseline ---------------------------------------
        opening = await service.monitors.brief_at(CALLER, first)
        ...
        claimed = await service.monitors.patch_lead(
            CALLER,
            "ANM-031",
            MonitorsLeadPatchRequest(status="resolved_claimed", note="reworked"),
            watermark=second,
        )
        assert claimed.status == "resolved_claimed"
        assert claimed.claimed_at_watermark == second.id

        moved = await service.monitors.monitors_at(CALLER, second)
        [tile_2] = moved.tiles
        ...
        history = await service.components.monitors_results.history(pin.pin_id)
