"""Rounds across real loads: wm_001 → wm_002 → wm_003, end to end.

Rounds is a load-over-load product, so the only test that proves it is one
...
* a watch is a SPEC re-run per load, not a snapshot, and its tile carries
...
from revi_api.rounds import typed_spec_from_analysis
...
#: A watch over a MOVING window, so a load-over-load delta is a real
...
class TestPinIsAWatch:
    ...
    async def test_a_pin_from_an_investigation_resolves_its_stored_spec(self) -> None:
        ...
        assert pin.label and pin.label != "Watched spec"

    async def test_an_absolute_window_is_pinned_as_one_and_says_what_that_means(
        self,
    ) -> None:
        """A watch over fixed dates re-measures the SAME period every load,
        so its delta is late-arriving data rather than a movement. Both are
        legitimate watches; only one of them is a movement."""
        service = _service()
        session = await service.open_session(CALLER, OpenSessionRequest())
        answer = await service.submit_turn(
            CALLER,
        ...
        pin = await service.rounds.create_pin(
            CALLER, CreateRoundsPinRequest(investigation_id=answer.investigation_id)
        )
    ...
    async def test_the_stored_spec_carries_scope_and_names_what_it_dropped(self) -> None:
        """A watch that silently dropped a scope clause would measure a
    ...
    async def test_a_pin_is_refused_for_another_tenant(self) -> None:
        service = _service()
        pin = await service.rounds.create_pin(
            CALLER, CreateRoundsPinRequest(spec=_moving_spec(), label="mine")
        )
        intruder = Principal(tenant="other", subject="intruder")

        with pytest.raises(ReviError) as refusal:
            await service.rounds.archive_pin(intruder, pin.pin_id)
    ...
    async def test_a_body_naming_both_sources_is_refused(self) -> None:
        ...
        with pytest.raises(PolicyDeniedError):
            await service.rounds.create_pin(
                CALLER,
                CreateRoundsPinRequest(investigation_id="inv_x", spec=_moving_spec()),
            )


class TestWatchThresholds:
    """A watch's own sensitivity, and the unit honesty that keeps it real."""
    ...
    async def test_a_points_threshold_on_a_money_contract_is_refused(self) -> None:
        ...
        nearest legal unit: a threshold in the wrong unit is a watch that
        ...
        assert "this watch measures 'money_cents'" in refusal.value.message
    ...
    async def test_a_points_threshold_on_a_rate_is_accepted(self) -> None:
        ...
        assert pin.watch is not None
        assert pin.watch.mode == "delta_gte" and pin.watch.unit == "points"
    ...
    async def test_a_threshold_mode_with_no_value_is_refused(self) -> None:
        ...
        with pytest.raises(PolicyDeniedError) as refusal:
            await service.rounds.create_pin(
                ...
                CreateRoundsPinRequest(
                    spec=_moving_spec(),
                    watch=RoundsWatchModel(mode="delta_gte"),
                ),
            )
...
class TestSimulatedLoads:
    ...
    async def test_the_first_load_says_it_is_the_first_load(self) -> None:
        ...
        brief = await service.rounds.brief_at(CALLER, first)
    ...
    async def test_a_tile_carries_the_integrity_line_and_a_permalink(self) -> None:
        ...
        adversarial rounds on the one surface a person looks at every
    ...
    async def test_evaluation_is_idempotent_per_load(self) -> None:
        ...
        watch against data that has not changed."""
        service = _service()
        first, _, _ = await _watermarks(service)
        pin = await service.rounds.create_pin(
            CALLER, CreateRoundsPinRequest(spec=_moving_spec(), label="denied dollars")
        ...
        await service.rounds.evaluate_load(TENANT, first)
        stored = await service.components.rounds_results.get(pin.pin_id, first.id)
        assert stored is not None
        await service.rounds.evaluate_load(TENANT, first)
        again = await service.components.rounds_results.get(pin.pin_id, first.id)

        assert again is not None
        assert again.evaluated_at == stored.evaluated_at, "a stored tile was recomputed"

    async def test_a_watched_spec_moves_across_loads_and_the_delta_is_unit_honest(
        self,
    ) -> None:
        service = _service()
        ...
        surface = await service.rounds.rounds_at(CALLER, second)
    ...
    async def test_a_brief_with_no_material_change_is_a_proud_answer(self) -> None:
        ...
        service = _service()
        first, second, _ = await _watermarks(service)
        # A watch over a period neither load moves: fixed dates, and the
    ...
    async def test_a_since_naming_an_unevaluated_load_is_refused(self) -> None:
        ...
        with pytest.raises(ReviError) as refusal:
            await service.rounds.brief_at(CALLER, third, since="wm_never_evaluated")
...
class TestLeadLifecycle:
    ...
    async def test_claiming_then_confirming_across_two_loads(self) -> None:
        ...
        claimed = await service.rounds.patch_lead(
            CALLER,
            "ANM-031",
            RoundsLeadPatchRequest(status="resolved_claimed", note="reworked by the AR team"),
            watermark=second,
        )
        ...
        await service.rounds.brief_at(CALLER, third)
        after_one = await service.rounds.get_lead(CALLER, "ANM-031")
    ...
    async def test_a_second_verifying_load_confirms_with_a_measured_sentence(self) -> None:
        ...
        first, second, third = await _watermarks(service)
        await service.rounds.brief_at(CALLER, first)
        await service.rounds.patch_lead(
            CALLER,
            "ANM-031",
            RoundsLeadPatchRequest(status="resolved_claimed"),
            watermark=first,
        )
    ...
    async def test_the_confirmation_sentence_names_what_it_measured(
        ...
        rounds_yaml = pack / "rounds.yaml"
        rounds_yaml.write_text(
            rounds_yaml.read_text().replace(
                "consecutive_loads_required: 2", "consecutive_loads_required: 1"
            ),
            encoding="utf-8",
        )
        service = _service(pack_dir=pack)
        assert service.components.rounds_policy.resolution.consecutive_loads_required == 1
        ...
        await service.rounds.patch_lead(
            CALLER,
            "ANM-031",
            RoundsLeadPatchRequest(status="resolved_claimed", note="rebilled"),
            watermark=second,
        )
        brief = await service.rounds.brief_at(CALLER, third)
    ...
    async def test_a_person_cannot_assert_a_confirmation(self) -> None:
        ...
        with pytest.raises(PolicyDeniedError) as refusal:
            await service.rounds.patch_lead(
                CALLER,
                "ANM-031",
                RoundsLeadPatchRequest.model_construct(status="resolved_confirmed"),
            )
    ...
    async def test_a_lead_not_in_the_feed_cannot_be_patched(self) -> None:
        ...
        with pytest.raises(ReviError) as refusal:
            await service.rounds.patch_lead(
                CALLER, "ANM-999", RoundsLeadPatchRequest(status="working")
            )
    ...
    async def test_lead_status_rides_onto_the_portfolio_cards(self) -> None:
        ...
        await service.rounds.patch_lead(
            CALLER,
            "ANM-021",
            RoundsLeadPatchRequest(status="working", note="Sarah has it"),
        )
