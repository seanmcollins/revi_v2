...
def test_every_shipped_contract_was_examined(
    ...
    # The restored pair: `exclusions:` names the population to REMOVE, so an
    # adjudicated-only population is `status eq OPEN`, never `neq`. Getting
    # this backwards is what shipped an un-adjudicated claim as a denial.
    for metric_id in ("denial_rate", "clean_claim_rate"):
        contract = base_pack_snapshot.metric(metric_id)
        assert contract is not None and contract.version == 2, metric_id
        exclusion = contract.exclusions
        assert isinstance(exclusion, Predicate), metric_id
        assert exclusion.dimension == DimensionRef("status"), metric_id
        assert exclusion.op is PredicateOp.EQ, metric_id
        assert exclusion.values == ("OPEN",), metric_id
    assert real_catalog.is_certified("status")
...
def test_a_regression_in_shipped_content_would_be_caught(
    ...
    check being vacuous."""
    # `status` is certified now, so the original example resolves. Pick a
    # dimension that is still absent — `submission_date` is a base-view
    # column and not a catalog dimension, which is exactly the mistaken
    # convention that produced the whole defect class.
    assert real_catalog.dimension("submission_date") is None
    broken = base_pack_snapshot.metric("clean_claim_rate")
    assert broken is not None
    regressed = replace(
        broken, exclusions=Predicate(DimensionRef("submission_date"), PredicateOp.IS_NULL, ())
    )
    ...
    assert ("clean_claim_rate", "submission_date") in excinfo.value.pairs
